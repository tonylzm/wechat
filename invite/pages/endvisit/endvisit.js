  ...
  loadDataFromBackend: function (status) {
    wx.request({
      ...
      success: (res) => {
        ...
        switch(status){
          case '即将访问':
            filteredData = processedData.filter(item => item.applicationStatus ==="通过");
            break;
          case '黑名单':
    ...
    if (status) {
      ...
      // 点击列表项，导航到目标页面并传递参数
        ...
        switch(application){
          case "拉黑":
            wx.navigateTo({
              url: '/pages/block-manage/block-manage?content=' + content,
          });
          break;
          default:
            wx.navigateTo({
              url: '/pages/Audit/Audit?content=' + content,
          });
          break;
