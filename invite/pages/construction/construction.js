// invite/pages/construction/construction.js
Page({
  data: {
    // 其他数据...
    visitReasons: ['施工人员'], // 下拉框选项
    useCar: true,
    elsepeople:false,
  ...
  togelsepeople:function(e){
    this.setData({
      elsepeople: e.detail.value,
    });
  },
  ...
  formSubmit: function(e) {
    ...
    formData.elsepeople=this.data.elsepeople;
  ...
  if(formData.elsepeople==true && !formData.elsepeopleinfo){
    wx.showToast({
      title: '请输入其他人员信息',
      icon:'error',
    });
  ...
  console.log(formData)
    wx.request({
      ...
      success: function(res) {
        ...
        wx.redirectTo({
          url: '../../../pages/loading/loading?visitorphone='+formData.visitorPhone,
        })
      }
      if(res.data==="该访客存在"){
        wx.showToast({
      ...
      fail: function(err) {
        console.log('Response from Server:', res); // 输出后端的响应
        console.error(err);
        wx.showToast({
          title: '提交失败',
          icon: 'none',
